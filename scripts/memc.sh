...
# Kill any stale tmux session matching our target experiment name
tmux kill-session -t "$TMUX_SESSION" 2>/dev/null || true

echo "Spawning isolated tmux registry..."
tmux new-session -d -s "$TMUX_SESSION" -n "memc"
...
tmux send-keys -t "$TMUX_SESSION:memc" "LD_PRELOAD=$SCRIPT_DIR/libreparent.so memcached -p 11211 -l 0.0.0.0 -u $USER -c 8192 -b 2048" C-m