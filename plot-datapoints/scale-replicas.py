...
apps = [
    {'title': 'YCSB A (50/50)', 'letter': 'A', 'lat_max': 60, 'lat_step': 20, 'tput_max': 4.0,  'tput_step': 1.0},
    {'title': 'YCSB B (95/5)',  'letter': 'B', 'lat_max': 40, 'lat_step': 10, 'tput_max': 10.0, 'tput_step': 2.0}
]

replica_counts = ['3servers', '5servers', '7servers']
x_labels = ['3', '5', '7']
x_indexes = np.arange(len(replica_counts))
...
# --- Build Layout ---
# Using manual tuning to split allocations perfectly between subplots and legend
...
for col_idx, app in enumerate(apps):
    ...
    for rep_str in replica_counts:
        for s in scheme_colors.keys():
            ...
            for c in range(1, TARGET_CLIENTS + 1):
                path = os.path.join("logs", "YCSB", f"workload-{app['letter']}", s, rep_str, f"{TARGET_CLIENTS}client", f"client{c}.txt")
                if not os.path.exists(path):
                    continue
    ...
    for ax in [lat_ax, tput_ax]:
        ...
        ax.set_xlim(-0.5, len(replica_counts) - 0.5)
    ...
    tput_ax.set_xlabel('Replicas', fontsize=8.5, labelpad=2)
...
# Clean overhead allocation tracking above the layout floor
...
fig.savefig("output-plots/replicas.pdf", format='pdf', bbox_inches='tight', pad_inches=0.01)
